...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
  ...
  if (choose == 'tài' || choose == 'xỉu') { 
    ...
    if (moneyUser < kqua) return api.sendMessage(`Số dư bạn không đủ ${kqua.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}$ để có thể chơi`, threadID, messageID);
if (choose == random) {
  ...
  await Currencies.decreaseMoney(senderID, parseInt(kqua ));
      return api.sendMessage(`Bạn thua bot lắc ra ${random} ${defl_number} và mất ${kqua.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`,event.threadID, event.messageID)}
 }
 if (choose == 'lẻ' || choose == 'chẵn') {
  ...
  return api.sendMessage(`Bạn thắng bot lắc ra ${random2} ${defl_number2} và nhận được ${kqua * 2}`,event.threadID, event.messageID)
   ...
   await Currencies.decreaseMoney(senderID, parseInt(kqua ));
  return api.sendMessage(`Bạn thua bot lắc ra ${random2} ${defl_number2} và mất trắng số tiền ${kqua.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`,event.threadID, event.messageID)}
  }
  if (choose == 'lode' || choose == 'lô' || choose == 'đề') { 
    if (kqua < 50 || isNaN(kqua)) return api.sendMessage("Mức đặt cược của bạn không phù hợp hoặc dưới 50$", threadID, messageID);
    if (moneyUser < kqua) return api.sendMessage(`Số dư bạn không đủ ${kqua.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}$ để có thể chơi`, threadID, messageID);
    api.sendMessage(`vui lòng đợi kết quả lô đề sau ít phút`, event.threadID, async (err, info) => {
      await new Promise(resolve => setTimeout(resolve, 120 * 1000));
      api.unsendMessage(info.messageID)
 if(kqua == x){
    await Currencies.inreaseMoney(senderID, parseInt(kqua * 2));
   return api.sendMessage(`Bạn đã thắng vì chọn ${args[1]} và kết quả lô hôm nay trên đài của bot là ${x} thần may mắn đã độ bạn và nhận được số tiền là ${kqua * 2}`, threadID, messageID)
 } else {
    await Currencies.decreaseMoney(senderID, parseInt(kqua ));
return api.sendMessage(`Bạn đã thua vì chọn ${args[1]} và kết quả lô hôm nay trên đài của bot là ${x} hic thần may mắn quên bạn rồi và mất số tiền là ${kqua.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`, threadID, messageID)
 }
    }
    )}
  ...
  return api.sendMessage(`Bạn thua bạn chọn là: ${kqua}\nsố thứ nhất bot chọn là: ${z}\nsố thứ nhất bot chọn là: ${y}\nhiệu số là ${dap_an} và mất số tiền là ${tiencuoc.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`,threadID, messageID)
  }
    }
  if (choose == 'baucua') {
  const slotItems = ["🍐", "🦀", "🐟", "🦌", "🐓", "🦞"];
...
for (let i = 0; i < 3; i++) number[i] = slotItems[Math.floor(Math.random() * slotItems.length)];
    ...
    api.sendMessage("Chờ tớ quay cái nha",event.threadID, event.messageID);
...
var array = [number[0],number[1],number[2]];
    if (array.includes(itemm)) {
        ...
        if (i == 1) {
            var mon = parseInt(args[1]) + 300; 
                await Currencies.increaseMoney(event.senderID, mon);
                    return api.sendMessage(`Kết Quả : ${array.join("|")}\nĐược ${mon.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}$ Vì Có 1 ${itemm}`,event.threadID, event.messageID);
        }
        else if (i == 2) {
            var mon = parseInt(args[1]) * 2; 
                await Currencies.increaseMoney(event.senderID, mon);
                    return api.sendMessage(`Kết Quả : ${array.join("|")}\nĐược ${mon.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}$ Vì Có 2 ${itemm}`,event.threadID, event.messageID);
        }
        else if (i == 3) {
            var mon = parseInt(args[1]) * 3; 
                await Currencies.increaseMoney(event.senderID, mon);
                    return api.sendMessage(`Kết Quả : ${array.join("|")}\nĐược ${mon.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}$ Vì Có 3 ${itemm}`,event.threadID, event.messageID);
        }
        ...
        await Currencies.decreaseMoney(event.senderID, parseInt(args[1]));
           return api.sendMessage(`Kết Quả: ${array.join("|")}\nTrừ ${args[1]}$ Vì Có 0 ${itemm}`,event.threadID, event.messageID);
 }
   }
 if (choose == 'slot') { 
   const slotItems = ["🍇", "🍉", "🍊", "🍏", "🥭", "🍓", "🍒", "🍌", "🥝", "🥑", "🌽"];
 ...
 module.exports.handleReply = async function ({
  ...
  const { threadID, messageID } = event;
    ...
    case "choosee": {
        switch (event.body) {
        case "1": { 
          return api.sendMessage(`Dùng ${global.config.PREFIX}casino [ TÀI OR XỈU ]`, threadID, messageID )
        }
        case "2": { 
          return api.sendMessage(`Dùng ${global.config.PREFIX}casino [ Chẵn OR Lẻ ]`, threadID, messageID )
        }
        case "3": { 
          return api.sendMessage(`Dùng ${global.config.PREFIX}casino [ Lode Or Lô Or Đề ] [ Số Mà Bạn Tin Tưởng ]`, threadID, messageID )
        }
        case "4": {
          return api.sendMessage(`Dùng ${global.config.PREFIX}casino [ hieu Or Hiệu Or Hieu ] [ Số Mà Bạn Tin Tưởng ]`, event.threadID, event.messageID )            }
         case "5": {
          return api.sendMessage(`Dùng ${global.config.PREFIX}casino [ Baucua ] [ Bầu, cua, cá, nai, gà, tôm ] [ Số tiền cần đặt cược ]`, event.threadID, event.messageID )}
        }
    }
    }
}
