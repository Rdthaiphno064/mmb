...
module.exports.config = {
	...
	try {
		...
		switch (r[0]) {
			...
			case "join":
			case "-j": {
				if (!global.chanle.has(s)) return e.sendMessage("Nhóm chưa mở bàn chãn lẻ", s, o);
				if (1 == g.start) return e.sendMessage("Hiện tại bàn game này đã bắt đầu từ trước", s, o);
				const r = await a.getData(n.senderID);
				if (r.money < g.money) return e.sendMessage(`Bạn không có đủ ${g.money}$ để tham gia bàn game này`, s, o);
			...
			default:
				return e.sendMessage({
					body: `[ Chẵn lẻ ]\n\n${global.config.PREFIX}${this.config.name} create -> tạo ván chẵn lẻ
${global.config.PREFIX}${this.config.name} join -> tham gia ván chẵn lẻ
${global.config.PREFIX}${this.config.name} leave -> rời ván chẵn lẻ
${global.config.PREFIX}${this.config.name} start -> bắt đầu ván chẵn lẻ
${global.config.PREFIX}${this.config.name} end -> kết thúc ván chẵn lẻ`,
					attachment: c
				}, s, o)
		}
	} catch (e) {
			...
			e.sendMessage({
				...
				setTimeout((async function() {
                    ...
                    function images() {
                        if ("chẵn" == t)
                            var i = [
    "https://i.imgur.com/wZVRYsk.png",
    "https://i.imgur.com/z3OjPul.jpg",
    "https://i.imgur.com/ydyUb8h.png",
    "https://i.imgur.com/NMFCtk9.png",
    "https://i.imgur.com/CIHsIYM.png"
														];
                        else if ("lẻ" == t)
                            i = [
    "https://i.imgur.com/DzioUMu.png",
    "https://i.imgur.com/aBFSH2C.jpg",
    "https://i.imgur.com/9fUbFAd.png",
    "https://i.imgur.com/1qBIEmd.png",
    "https://i.imgur.com/fSM4trA.png"
														];
                        return i;
                    }
				}), 5000)
