...
module.exports.handleEvent = async function({ api, event, Currencies }) {
    ...
    if(body.toLowerCase() == 'my pokemon') {
        ...
        if (q.choose.status == true) return api.sendMessage('⚠ Bạn vừa chọn rồi không thể chọn lại', threadID, messageID);
        var msg = `🔍 𝐒𝐨̂́ 𝐏𝐨𝐤𝐞𝐦𝐨𝐧 𝐡𝐢𝐞̣̂𝐧 𝐜𝐨́ ${user.pet.length}\n`
        var ii = 0;
        for (let i of user.pet) {
            msg += `[${++ii}]. ${i.name} - ${i.coins}$\n👾 𝐓𝐲𝐩𝐞: ${i.type}\n🧡 HP: ${fixed(i.HP)}\n🗡 ATTACK: ${fixed(i.Attack)}\n🛡 DEFENSE: ${fixed(i.Defense)}\n⚡️ SPEEED: ${fixed(i.Speed)}\n📌 SKILL: ${i.skill.join(', ')}\n\n`
        }
        msg += 'Nhớ số thứ tự pokemon bạn vừa chọn'
        api.sendMessage(msg, senderID)
        return api.sendMessage('Reply (phản hồi) tin nhắn này theo stt để chọn pokemon', threadID, (error, info) => {
...
module.exports.run = async ({ api, event, args, Users }) => {
    ...
    switch (args[0]) {
        ...
        case "-r": { 
            if (!existsSync(path)) {
                ...
                return api.sendMessage("====[ POKEMON ]====\n⚔️Nhận nuôi pokemon thành công⚔️", threadID, messageID);
            }
            else return api.sendMessage("====[ POKEMON ====\n⚔️Bạn hiện có mặt trong hồ sơ rồi⚔️", threadID, messageID);
            break;
        }
        case 'rank': {
        ...
        case '-i': {
            ...
            return api.sendMessage(`====[ INFO USER ]====\n\n👤 Người nuôi pokemon: ${name}\n🔍 ID: ${ID}\n⏳ Số pokemon: ${pet}\n🛍 Túi thức ăn: ${foods}\n🔰 Số trận thắng: ${win}\n📛 Số trận thua: ${lose}\n\nThả cảm xúc '👍' nếu muốn xem pokemon hiện có`, threadID, (error, info) => {
  ...
  const { threadID, messageID } = event;
   return api.sendMessage({body: "===[ POKEMON ]===\n\n𝟭. Mua thức ăn cho pokemon\n𝟮. Bán pokemon\n𝟯. Cường hóa sức mạnh (bỏ ra 70,000$ để tăng 40% sức mạnh cho 1 pokemon).\n𝟰. Mua rương hextech ra ngẫu nhiên 1 pokemon\n\nReply (phản hồi) tin nhắn này theo stt để chọn",attachment: (picture)
